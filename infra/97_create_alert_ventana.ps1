# infra/97_create_alert_ventana.ps1
#
# Crea la alerta de VENTANA DE NEGOCIO del datamart (F-025): avisa cuando el
# guardian `check-ventana` encuentra una obra congelada que se ha quedado vieja
# sin que nadie se entere.
#
# POR QUE HACE FALTA UNA TERCERA ALERTA. 90_create_alert.ps1 dispara cuando una
# ejecucion TERMINA EN FALLO, 95_create_alert_frescura.ps1 cuando el job NO
# LLEGA A HACER su trabajo y 96_create_alert_cobertura.ps1 cuando una obra entra
# en `stg` y no sale en `mart`. Ninguna cubre el caso de F-025: desde el
# 2026-09-03 el datamart NO reconstruye las 920 obras cada noche -se rehacen 40
# y 880 conservan su ultima version buena-, asi que la noche va bien, el job
# termina en verde, la obra esta en el datamart... y su dato es de hace
# semanas. Es el mismo modo de fallo de F-052 con otra cara: UN DATO QUE
# ENVEJECE Y DEL QUE NADIE SE ENTERA.
#
# QUE DENUNCIA. Cuatro cosas, y son cuatro maneras distintas de que eso pase:
# una obra congelada cuya FIRMA DEL ORIGEN ha cambiado -se movio en Sigrid-,
# una obra congelada SIN FILAS construidas, una obra con un SELLO DEL SQL que
# ya no es el vigente -le falta el ultimo arreglo- y la RECONSTRUCCION COMPLETA
# VENCIDA, que es el domingo que no corrio.
#
# SIN ESTA REGLA EL GUARDIAN ES MUDO, y es el precio declarado de DA-5. Se
# decidio que `check-ventana` AVISE Y NO BLOQUEE: la nocturna termina en verde
...
# el guardian detecta la obra vieja, la escribe en el log y no se entera NADIE.
#
...
# [F025-VENTANA-KO] y dispara si hay AL MENOS UNA. Ojo, es AL REVES que la de
...
# etl_sigrid/domain/ventana.py (constante MARCADOR_KO). Lo cruza
# tests/test_f025_marcador.py, que ademas EJECUTA la funcion de composicion de
...
# enteraria, que es exactamente el modo de fallo que F-025 existe para eliminar.
#
# LA VENTANA. 24 h, que es `ventanaVentanaHoras` en infra/env/<entorno>.json:
...
function Componer-ConsultaVentana {
    <#
    .SYNOPSIS
    ...
    .DESCRIPTION
        ...
        El nombre por defecto, 'Ventana', es el mismo que cuenta el argumento
        --condition ("count 'Ventana' > 0"). Son dos argumentos distintos que
    ...
    param(
        ...
        [string]$Nombre = "Ventana"
    )

    # EL LITERAL. Tiene que ser byte a byte el MARCADOR_KO de
    # etl_sigrid/domain/ventana.py. Lo cruza tests/test_f025_marcador.py.
    $marcador = "[F025-VENTANA-KO]"
...
$ventana = "{0}h" -f [int]$CFG.ventanaVentanaHoras
...
$consulta = Componer-ConsultaVentana -Job $CFG.job

$yaExiste = Invoke-Az monitor scheduled-query show `
    -g $CFG.resourceGroup -n $CFG.ventanaAlertName --query id -o tsv

if ($LASTEXITCODE -eq 0 -and $yaExiste) {
    Write-Host "La alerta '$($CFG.ventanaAlertName)' ya existe; no se recrea." -ForegroundColor Yellow
} else {
    Write-Host "Creando la alerta de ventana '$($CFG.ventanaAlertName)'..." -ForegroundColor Cyan
    ...
    Invoke-Az monitor scheduled-query create `
        -g $CFG.resourceGroup -n $CFG.ventanaAlertName `
        --location $CFG.location `
        --scopes $idWorkspace `
        --condition "count 'Ventana' > 0" `
        ...
        --auto-mitigate true `
        --action-groups $idAg `
        --description "Una obra congelada del datamart se ha quedado vieja: firma del origen cambiada, sin filas, sello no vigente o reconstruccion completa vencida" `
        --tags @(Get-EtiquetasCli) -o none
    Confirmar-Exito "no se ha podido crear la alerta de ventana"
}
...
Write-Host ""
Write-Host "Alerta de ventana lista." -ForegroundColor Green
Write-Host "NO esta verificada hasta que llegue un correo de verdad." -ForegroundColor Yellow
Write-Host "Y hasta entonces el guardian es MUDO: check-ventana no tumba el job,"
