...
`infra/sql/02_roles.sql` y `.env.example`. **Tests**:
`test_f074_ingesta_censo.py` (nuevo, 119), mas `TOTAL_TABLAS` y la excepcion de
`prvcer` en `test_f066_ingesta_raw.py` y la lista de cuatro en
...
`config/settings.py`, y solo esa constante. Esta feature no necesitaba codigo,
necesitaba que lo declarado fuera cierto.
...
**Y no es interpretacion mia: el repositorio ya lo tenia aprendido a golpes.** El
docstring de `tests/test_f006_raw_ingesta.py` cuenta que F-006 se equivoco DOS
VECES en esta misma afirmacion —la 7a pasada derivandola de `tables_sigrid.yaml`
y la 8a de `ingest_raw_step.py`— y que por eso nacio
`tests/test_f006_fuente_que_gobierna.py`, que ancla el hecho al `Dockerfile`. El
riesgo 1 de `progress/explore_F-074_las_nueve.md` repite ese error por tercera
...
Entran con el recuento medido escrito en el YAML **y** en su ficha; `ide` es
`int` y unico en las nueve. Con `tiemod`: `auxdpt` (7 filas), `auxhor` (60) y
`auxrestip` (37). Sin `tiemod`: `cet` (40, excluye 7), `pro` (55.179, excluye 4),
`reshor` (8.949), `emphis` (1.633, excluye 1), `dcaprodes` (850.985) y
`ctrprodes` (424.475). Por que entra cada una: `config/tables_sigrid.yaml` y
`docs/ARCHITECTURE.md`.

**Ninguna fija `page_size`**, y es deliberado: el global ya vale 10.000, justo lo
que la exploracion proponia escribir a mano; fijarlo no cambiaria nada hoy y las
dejaria pinchadas el dia que haya que bajar el global. **Una correccion al informe de
exploracion**: el total de filas nuevas son **1.341.365**, no 1.341.469 (error
aritmetico mio, cazado por el test de control de la propia feature).
...
3. **`obrprv` SE QUEDA**, con el motivo en el YAML. Cuesta una peticion HTTP que
   ...
   que ha dejado de estarlo. Sacarla convertiria un hecho vigilado en una
   suposicion de 2026. Un test fija esos dos SQL.
...
verdad aplica aqui**: estas dos tablas **todavia no existen** en Azure, y
`test_f074_r3_el_revoke_sobrevive_a_que_la_tabla_no_exista_aun` comprueba con
`missing_tables` que la regla del catalogo se sigue revocando —el agujero que
cazo el review de F-068 en su pasada 1—. La verificacion **contra la base** es
MANUAL (§7).
...
Y los heredados, tambien puestos en rojo antes de implementar:
...
contra Azure o dependen de que la imagen nueva se haya desplegado y la nocturna
haya corrido**, asi que ninguna la puede ejecutar un agente.
...
3. La consulta a `information_schema.table_privileges` que tiene que devolver
   **cero filas** para `emp`, `res`, `reshor` y `emphis` (criterio 3).
...
* **`obrprv` conserva un comentario heredado** que le atribuye `cod` y `res`: con
  0 filas no se puede comprobar. Y **el MCP no lee `raw`**: las nueve fichas no
  cambian ninguna respuesta de la IA hoy.
...
**Coherencia temporal (RM2, corregida por workers)**: media 78,2 s × 4 workers =
**312,8 s de coste real por mutante**, por encima de la linea base de 232-236 s.
Coherente: la campaña evalua con `-x` y los 12 muertos abortan la suite en el
primer fallo, lo que baja la media de pared; y `20 × 78,2 = 1.564 s`, el tiempo
total, por construccion. **Sin cabecera «CAMPAÑA NO VALIDA» y sin «Sin
veredicto» distinto de cero.**

**Aviso de alcance (RM1).** El informe declara **14 ficheros y 3.527 lineas de
produccion**, y eso **no** es lo que toca F-074: el diff sale del `merge-base` con
`dev` (`cd18e096`), que arrastra F-025, F-066 y F-068 enteras. Esta feature toca
de produccion **una constante de `config/settings.py`**; el SHA es el de esta rama.

### Supervivientes: 8, ninguno en `PENDIENTE`

Analisis completo, uno a uno, en **`progress/mutacion_F-074.md`**:

* **Siete son de F-025** —seis en `PostgresClient`, uno en `ventana_sql.py`— y
  **no se tapan aqui**. Ninguno es equivalente, y el informe dice de cada uno que
  rompe. Sobreviven por la misma razon estructural: son el cuerpo de metodos que
  abren cursor, y la suite **no toca red ni BBDD** por convencion, asi que esas
  lineas no se ejecutan ni una vez. Taparlos exige tests de integracion contra un
  Postgres real: alcance del humano, no apaño de esta feature. **Escritos para
  que se puedan fichar.**
* **El octavo si era nuestro, y era el peor**: `main.py:543`, `is_flag=True ->
  is_flag=False` sobre `--full` de `run-all`, la bandera de la que depende entera
  la decision de carga de F-074. Sobrevivia porque el test que la vigilaba
  comprobaba que la opcion **existe** —expresion regular sobre el texto— y no
  **que es**. **Cerrado con un test nuevo y no quitando codigo** (RM6):
  `test_f074_r2_la_bandera_full_de_run_all_es_un_flag_booleano`, con la traza del
  arbol mutado a mano —y restaurado— en `progress/mutacion_F-074.md`.
