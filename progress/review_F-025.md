...
`maestro.yaml` y `business_rules.yaml`. Fallan cuatro residuos (§Cambios 3).
...
pronunciamiento del humano»**; pero en `critico` `rigor.json` dice `max_mutantes:
null` = *«aquí se mide la campaña entera»* y solo se exime con **justificación
escrita aceptada por el humano**, y aquí se la concedió la spec a sí misma. **Es
mi fallo de la pasada 1**, por no recalcular el alcance. Cerrarlo: ~**2 h 40**.
...
reejecutada: 7.720 s (2 h 8) según el informe**, sobre el umbral de 60 s. **RM2
coherente**: `83 × 93,0 = 7.720` y `media × 4 workers = 372 s` contra una base de
467-473 s, por debajo como manda el `-x`. **RM3** sin caso; **RM5/RM6 `N/A`
justificados**: ni equivalentes declarados ni código defensivo retirado.
...
aborta pronto con `-x`). Los reproduje **sobre una copia de HEAD en mi scratchpad**
...
`test_f025_r10_el_build_NO_llama_a_truncate...` y `..._r30_ventana_plan_...`:
...
- **C4** `[x]` — las once MANUAL de `current.md` traen su comando **literal**, la
  precondición delante y el criterio de parada de T1; la trazabilidad
...
- **C4 bis** `[ ]` — RED `[x]`, cobertura `[x]`, RM1-RM4 `[x]` (RM4, mío),
  ...
  (38 % de los mutantes, sin exención del humano) y **«Evidencias»**, que publica
  los números de la campaña **descartada**.
...
3. **Las cuatro cifras viejas del censo, dichas como hechos:**
   ...
   declaran debajo. `business_rules.yaml` **es** la regla de negocio: quien la
...
diferencias que explicar una a una: **el defecto de F-052 con otra ropa**.
...
reducción exige **exención escrita del humano**, no de la spec —sin esto se
aprueba una campaña que mide el 38 % y cuyos números cuadran—. **(2) Los timeouts
no son muertos**: C4 bis exige «cero supervivientes» y calla sobre los que no
tienen veredicto por reloj; **cada uno se cierra por RM4 o se repite la campaña**.
Y una menor, ajena a F-025: `test_f015_r4_ejecutar_git_de_verdad` es sensible a
la carga, porque `ejecutar_git` no pone timeout y devuelve `""` ante rc≠0.
