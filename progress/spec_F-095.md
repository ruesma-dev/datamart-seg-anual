...
(requirements 150/150, design 242/250, 26 tareas T0-T25). Estado de la ficha:
sigue `pending` hasta que el humano apruebe la spec y conteste H1-H7.
...
La decision del humano del 2026-09-22 esta escrita como R21: **el vencimiento es
...
## Decisiones abiertas que necesita validar el humano (detalle en design.md)

- **H1** Fin de obra: recomendado fin real -> recepcion provisional (30,6 % del
  vivo); fin previsto NO; 83 obras terminadas sin fecha (2,09 M€) sin
  vencimiento y declaradas, con la lista a Administracion.
- **H2** Plazo: recomendado `plaret` -> `plagar` del cliente -> plazo fijo de
  Negocio. **Falta que Negocio diga el valor** (12 dominante; Sigrid usa 15).
- **H3** Bajas sin obra: recomendado `SIN_OBRA` + `PROVEEDOR_UNA_OBRA`; FIFO no.
- **H4** `rac`: recomendado filtrada (~1 min, 90-120 MB estimados), acordada con
  F-091; entera 2,5-4,5 min y 300-400 MB.
- **H5** Cliente: recomendado fuera, feature propia tras F-094.
- **H6** F-059: retirar como absorbida. F-045: su resto (traducir
  `movimientos.obra_id` de centro a obra) a F-094, y retirarla.
- **H7** Descuadre 3,28 M€: publicar categoria por proveedor (reparto medido en
  design) y pasar a Administracion la lista de mayores descuadres.
