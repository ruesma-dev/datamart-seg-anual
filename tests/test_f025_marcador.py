# tests/test_f025_marcador.py
"""
F-025 · El marcador `[F025-VENTANA-KO]`, fijado por sus DOS extremos (R26-R27).

**Por qué esto es un fichero de tests y no un comentario.** `check-ventana`
avisa y **no bloquea** (DA-5): la nocturna termina en verde, así que la alerta
de fallo existente (`alert-caj-datamart-seg-dev-failed`) no se dispara. La
**única** vía por la que este guardián se hace oír es una línea de log con un
literal fijo que una regla de consulta programada busca. Si el literal del
código y el del `.ps1` divergen, la regla vigila un texto que ya nadie escribe y
**nadie se entera** — que es, exactamente, el modo de fallo que esta feature
existe para eliminar.

Y aquí importa el doble: desde F-025 el datamart **no reconstruye las 920 obras
cada noche**. Se rehacen 40 y 880 conservan su última versión buena, así que una
obra puede estar en el datamart, con el job en verde, y tener el dato de hace
semanas. Lo único que lo cuenta es este marcador.

Es el mismo patrón con el que `test_f052_marcador.py` protege el de cobertura y
con el que `test_f024_infra_alerta.py` cruza el evento `step_finished` con la
consulta que lo busca: **los dos extremos, en el mismo fichero**, para que lo
que se rompa sea la suite y no la alerta a las 02:00 de un martes.

Los helpers de lectura de `infra/` se **importan** de `test_f003_infra.py`, no
se copian: dos copias de un barrido de secretos divergen, y ya pasó en este
repositorio.
...
SCRIPT = "97_create_alert_ventana.ps1"
...
FUNCION_CONSULTA = "Componer-ConsultaVentana"
...
def test_f025_r27_el_marcador_del_ps1_es_el_MISMO_que_emite_el_codigo():  # noqa: N802
    ...
    assert MARCADOR_KO in _script(SCRIPT), (
        ...
        f"etl_sigrid/domain/ventana.py. Con eso la regla vigila un texto que "
        f"nadie escribe y el guardián se queda mudo"
    )


@pytest.mark.skipif(POWERSHELL is None, reason="no hay powershell en este puesto")
def test_f025_r27_el_marcador_llega_a_la_consulta_que_se_envia_de_verdad():
...
@pytest.mark.skipif(POWERSHELL is None, reason="no hay powershell en este puesto")
def test_f025_r27_la_alerta_mira_el_job_por_la_columna_real():
...
def test_f025_r27_el_nombre_de_la_consulta_es_el_que_cuenta_la_condicion():
...
def test_f025_r27_el_codigo_emite_el_marcador_cuando_hay_hallazgos():
    """El otro extremo del contrato: que el ETL escriba de verdad esa línea."""
    import inspect

    fuente = inspect.getsource(main._emitir_marcador_de_ventana)
...
def test_f025_r27_el_marcador_es_estable_y_buscable():
    ...
    assert MARCADOR_KO == "[F025-VENTANA-KO]"
...
def test_f025_r27_el_script_no_lleva_ninguna_direccion_de_correo():
...
def test_f025_r27_el_script_lee_todo_de_cfg_y_no_cablea_ningun_nombre():
    texto = _script(SCRIPT)
    ...
    cfg = _config("dev")
    for clave in ("resourceGroup", "logAnalytics", "job", "ventanaAlertName",
...
def test_f025_r27_el_script_notifica_al_grupo_de_accion_que_ya_existe():
...
def test_f025_r27_la_alerta_es_idempotente():
...
def test_f025_r27_la_ventana_sale_del_entorno_y_no_es_iso_8601():
    """`az` acepta `##h##m##s` y rechaza un `PT24H`, y el valor sale de
    `ventanaVentanaHoras`: no se escribe a mano."""
    texto = _script(SCRIPT)

    assert "$CFG.ventanaVentanaHoras" in texto
...
def test_f025_r27_dev_json_declara_el_nombre_y_la_ventana():
    cfg = _config("dev")

    assert cfg["ventanaAlertName"]
    assert cfg["ventanaVentanaHoras"] == 24, (
        "el guardián corre una vez por noche: la ventana tiene que cubrir una"
    )
    assert not re.search(
        PATRONES_PROHIBIDOS["dirección de correo"], cfg["ventanaAlertName"]
    )


def test_f025_r27_el_script_esta_en_la_lista_de_los_ps1():
...
def test_f025_r27_utf8_bom_crlf_y_cabecera_de_ruta():
...
def test_f025_r27_ninguna_llamada_a_az_va_suelta():
...
def test_f025_r27_el_readme_documenta_el_script_despues_del_95():
    ...
    assert readme.index("96_create_alert_cobertura.ps1") < readme.index(SCRIPT)


def test_f025_r27_el_script_dice_que_sin_desplegarlo_el_guardian_es_mudo():
