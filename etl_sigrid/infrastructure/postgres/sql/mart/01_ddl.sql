...
-- Importe pre-calculado.
--
-- IMPORTANTE - precisión del PRECIO:
--   Sigrid almacena `pre` en double precision con hasta 6 decimales reales
--   (ej: 115.294961). Si guardamos `precio` con menos decimales (p.ej.
--   NUMERIC(18,4)), el ROUND(precio, 2) posterior puede divergir del
--   ROUND(pre_crudo, 2) cuando el 5º decimal del precio es 5..9.
--
--   Caso real obra 0696 partida P4.04.01.02:
--     pre crudo                     = 115.294961
--     ROUND(pre crudo, 2)           = 115.29  ← correcto
--     pre en NUMERIC(18,4)          = 115.2950 (truncamiento)
--     ROUND(115.2950, 2)            = 115.30  ← se va 0.01 € arriba
--   Resultado: residual de 1,70 € en Venta Real mayo 2025 (170 unidades × 0.01).
--
--   Solución: NUMERIC(20,6).
--   - Escala 6: preserva los 6 decimales reales de Sigrid
--   - Precisión total 20: mantiene 14 dígitos enteros (igual que NUMERIC(18,4)
--     original), evitando overflow en precios anómalos (>10^12) que pueda
--     haber en Sigrid por errores de captura.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS stg.presupuesto (
    ...
    precio          NUMERIC(20,6),                  -- 6 decimales para precisión Sigrid + 14 enteros para no overflow
...
-- Migración defensiva: si la tabla ya existía con precio NUMERIC(18,4) o
-- NUMERIC(18,6), migrarla a NUMERIC(20,6). Idempotente: si ya tiene los
-- 20 dígitos de precisión y 6 de escala, no hace nada.
DO $$
DECLARE
    v_precision INTEGER;
    v_scale     INTEGER;
...
--   Sigrid almacena `pre` en double precision con hasta 6 decimales reales,
--   pero internamente redondea a 2 decimales para todos los cálculos
--   visibles en la UI de Seguimiento. Para que nuestro mart cuadre con
--   Sigrid céntimo a céntimo, guardamos:
...
--   El campo precio_unitario se guarda CRUDO (NUMERIC(20,6)) para trazabilidad
--   y para que el ROUND(2) aplicado downstream sea correcto.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS stg.plan_mensual (
    ...
    precio_unitario      NUMERIC(20,6) NOT NULL,           -- pre crudo de obrparpre (6 dec para preservar precisión)
...
-- Migración defensiva para precio_unitario en plan_mensual: misma lógica.
DO $$
DECLARE
    v_precision INTEGER;
    v_scale     INTEGER;
BEGIN
    ...
    FROM information_schema.columns
    WHERE table_schema = 'stg'
      AND table_name   = 'plan_mensual'
      AND column_name  = 'precio_unitario';

    IF v_precision IS NOT NULL AND (v_precision < 20 OR v_scale < 6) THEN
        ALTER TABLE stg.plan_mensual ALTER COLUMN precio_unitario TYPE NUMERIC(20,6);
        RAISE NOTICE 'Migrado stg.plan_mensual.precio_unitario de NUMERIC(%, %) a NUMERIC(20, 6)',
                     v_precision, v_scale;
    END IF;
END $$;