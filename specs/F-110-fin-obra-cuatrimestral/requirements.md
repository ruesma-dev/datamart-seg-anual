...
la obra + 1 mes; (3) si tampoco, SIN FECHA. **Se elimina el respaldo «ultimo
cierre con movimiento + 1 mes»**. Plazo y vencimiento no cambian.
**[Dn]** = decision abierta que el humano valida (`design.md` §Decisiones
abiertas); cada R la escribe con la opcion RECOMENDADA.

Sustituye a R18, R19 (solo el respaldo) y R21 de F-095. R17 (garantia e
informativas), R20, R22, R23, R24 y R25 de F-095 siguen en vigor.
...
10 de las 117 obras: `design.md` D1).
...
el mayor `anio_mes` de `stg.plan_mensual` con el `obra_id` de la obra,
`version = version_cuatrimestral`, `ambito_id IN (8, 11)` e `importe_mes <> 0`:
el ultimo mes en que esa version planifica algun movimiento, de coste o de venta.
...
R7. El sistema no debe tomar el ultimo mes con fila sin movimiento: los meses de
cola que Sigrid arrastra con el porcentaje congelado (`importe_mes = 0`) no son
plan (en 15 de 117 obras alargarian el fin hasta 10 meses).
...
R10. **[D5]** El sistema no debe leer ningun objeto de `cierre` en
`05_fin_obra.sql` ni publicar la columna `ultimo_cierre`: el ultimo cierre deja
de intervenir y de publicarse.
...
`terminada_sin_fin_obra` sigue marcando las de `con.est` 19, 21, 23 y 25.

R12. El sistema no debe usar en el fin de obra `fecha_fin_real`,
`fecha_recepcion_provisional` ni `fecha_fin_prevista`, que siguen informativas.
...
los ambitos 8 u 11, ENTONCES el sub-paso debe fallar con un mensaje que empiece
por `fin_obra:` ANTES de dropear la tabla publicada (sustituye a R21 de F-095).

R15. **[D4]** `BuildRetencionesStep.depends_on` debe seguir siendo
`["ingest_raw"]`: un fallo de `build_stg` o `build_mart` no deja la noche sin
retenciones; `fin_obra` usa entonces las tablas de la noche anterior, que no se
borran.
...
docstring del step y el comentario de `depends_on` dejan de citar
`cierre.fact_cierre_mensual` y citan las dos lecturas nuevas.
...
debe explicar la regla de R8 y la decision del humano del 2026-09-25, publicar
las dos columnas nuevas, retirar `ultimo_cierre` y traer las cifras medidas
antes y despues por fuente (las de F-095 quedan como historicas y con fecha).
