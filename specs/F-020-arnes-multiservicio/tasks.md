...
- [x] T1: `harness/servicios.py` — dataclass `Servicio`, `cargar_servicios`
...
- [x] T2: `harness/alcance.py` — `es_produccion()` excluye
...
- [x] T3: `harness/cobertura.py` — `fusionar_coberturas()` y carga opcional
...
- [x] T4: `harness/mutacion.py` — `EjecutorPytest(cwd, ejecutable)`, factoría
      `ejecutor_para()`, exit 5 de pytest = SUPERVIVIENTE.
      | Verificación: `pytest tests/ -k "f020_r15 or f020_r16 or f020_r2_mutacion" -q`
- [x] T5: `harness/init.sh` — sección multi-servicio condicionada a
...
- [x] T6: test de genericidad de las herramientas (sin Sigrid, capas del
      datamart, Azure ni nombres de apps).
      | Verificación: `pytest tests/ -k f020_r17 -q`
- [x] T7: portado a `arnes-base`: `harness/*.py` e `init.sh` actualizados,
