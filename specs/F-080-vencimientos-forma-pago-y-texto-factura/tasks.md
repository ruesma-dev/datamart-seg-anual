...
- [ ] T0: Verificar la precondición DURA de F-073 en el árbol: que `sql/compras/04_formas_pago.sql` existe y que `etl_sigrid/application/steps/build_compras_step.py` lo declara como sub-paso; anotar el resultado en `progress/impl_F-080.md`  |  Verificación: `ls etl_sigrid/infrastructure/postgres/sql/compras/04_formas_pago.sql` y `grep -n formas_pago etl_sigrid/application/steps/build_compras_step.py` devuelven resultado; si falta cualquiera de las dos, `blocked` en `progress/current.md` y parada
