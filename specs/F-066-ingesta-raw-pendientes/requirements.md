...
- **R1.** El sistema debe declarar las **24** tablas nuevas de `design.md`
...
- **R5.** CUANDO se ingiera `emp`, el sistema debe excluir las **72**
  columnas de `design.md` §3 (identidad legal, bancarias, personales,
  domicilio y contacto, credenciales, binario y texto): entre ellas `dni`,
  `dnipai`, `tarseg`, `fecnac`, `ban`, `bancue`, `ima`, `esigpas`, `g3wpas`.
  Se conservan nombre y apellidos, alta y baja, y los enlaces.
- **R6.** CUANDO se ingiera `res`, el sistema debe excluir `cif`, `cifpai`,
  `tipcif`, `logacc`, `ideacc`, `ipacc` y `recema`.
...
- **R11.** La ficha de `raw.emp` debe citar `dni` y `tarseg` en la frase «No
  se traen» y la de `raw.res` debe citar `cif`; la de `raw.dcf` debe pasar de
  «No se traen 23» a 21.
...
- **R13.** CUANDO cambie `raw.yaml`, el sistema debe subir `version` en
  ...
  `00_global.yaml` y `objetos_pendientes.yaml` a **55**. Un test compara el
...
- **R14.** Las fichas de `confir`, `conact`, `dco`, `ctrrec` y `dcfrec` deben
  ...
  cambio de estado de un contrato. Un test busca «cambio de estado» en la
  ficha de `confir`.
...
- **R19.** El sistema debe dejar en `specs/F-066-ingesta-raw-pendientes/
  mediciones.md` la **primera nocturna** con las 24 tablas: por tabla, filas
...
- **R21.** CUANDO se cierre la feature, `azure-apps/datamart_seg_anual.md`
  debe decir que se consumen **55** tablas, listar las 24 nuevas y las
  descartadas por vacías, y decir que `dcf` trae ya `pagtex`/`pagfor`.
- **R22.** El líder debe llevar a `harness/features.json` los hallazgos que
  ...
  histórico por foto diaria en el datamart.
