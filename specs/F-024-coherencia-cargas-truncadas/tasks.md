...
- [ ] **T5**: `etl_sigrid/domain/ejecucion.py` (`Ejecucion`,
...
- [ ] **T6**: `etl_sigrid/domain/coherencia.py` (`EstadoTablaRaw`,
...
- [ ] **T7**: DDL de `_meta` (`00_meta.sql`: `batch_id`, índice,
...
- [ ] **T8**: `postgres_client.py`: `batch_id` opcional en
...
- [ ] **T9**: `timings.py`: `format_timings(timings, ahora)` con el aviso
...
- [ ] **T10**: `ingest_raw_step.py` (`batch_id`) y `build_stg_step.py`
...
- [ ] **T11**: (DA-5) `build_mart_step.py` con `_puerta_stg` y registro
      `build_mart.puerta_stg`. Tests R15. | Verificación:
      `pytest tests/test_f024_steps.py -k r15`.
- [ ] **T12**: `main.py`: `_arrancar_ejecucion`, `_ejecutar_paso`, los
...
- [ ] **T13**: (DA-3 = A) `infra/95_create_alert_frescura.ps1`,
...
- [ ] **T14**: Documentación: `docs/ARCHITECTURE.md` (sección F-024 y
...
- [ ] **T15**: Campaña de mutación sobre el alcance de la feature e
...
- [ ] **T16**: Ejecutar `bash harness/init.sh` en verde (pytest completo,
