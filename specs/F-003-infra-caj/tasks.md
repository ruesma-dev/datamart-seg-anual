...
- [ ] **T1**: Crear `infra/env/dev.json` con todas las claves obligatorias de
...
- [ ] **T2**: Escribir `tests/test_f003_infra.py` con los tests de R1–R6:
...
- [ ] **T3**: Reescribir `infra/00_vars.ps1`: `param(-Entorno)`, resolución
...
- [ ] **T4**: Crear `infra/05_check_prereqs.ps1` (solo lectura): sesión de `az`,
...
- [ ] **T7**: `infra/40_create_storage.ps1` — cuenta con
...
- [ ] **T12**: Crear `etl_sigrid/infrastructure/postgres/entra_auth.py` con
...
- [ ] **T13**: Modificar `config/settings.py`: `auth_mode`, `sslmode`,
...
- [ ] **T14**: Añadir `azure-identity>=1.17.0` a `requirements.txt` **si F-004 no
